...
class PLL_MO extends MO {
	...
	public function export_to_db( $lang ) {
		...
		$strings = wp_slash( $strings ); // Avoid breaking slashed strings in update_post_meta. See http://codex.wordpress.org/Function_Reference/update_post_meta#Character_Escaping

		if ( empty( $lang->mo_id ) ) {
			$post = array(
				'post_title'  => 'polylang_mo_' . $lang->term_id,
				'post_status' => 'private', // To avoid a conflict with WP Super Cache. See http://wordpress.org/support/topic/polylang_mo-and-404s-take-2
