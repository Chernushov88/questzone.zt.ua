...
class PLL_Frontend_Filters_Links extends PLL_Filters_Links {
	...
	public function wp_head() {
		// Don't output anything on paged archives: see http://wordpress.org/support/topic/hreflang-on-page2
		// Don't output anything on paged pages and paged posts
		if ( is_paged() || ( is_singular() && ( $page = get_query_var( 'page' ) ) && $page > 1 ) ) {
			return;
		}

		// Google recommends to include self link http://support.google.com/webmasters/answer/189077?hl=en
		...
		if ( ! empty( $urls ) && count( $urls ) > 1 ) {
			...
			// See http://wordpress.org/support/topic/implementation-of-hreflangx-default
